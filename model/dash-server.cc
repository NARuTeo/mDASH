  ...
  DashServer::GetTypeId(void)
  {
    static TypeId tid =
        TypeId("ns3::DashServer").SetParent<Application>().AddConstructor<DashServer>()
            ...
            .AddAttribute("Protocol", "The type id of the protocol to use for the rx socket.",TypeIdValue(UdpSocketFactory::GetTypeId()),
                MakeTypeIdAccessor(&DashServer::m_tid), MakeTypeIdChecker())
            .AddAttribute("Bw", "Server's export bandwidth", UintegerValue(0),
                MakeUintegerAccessor(&DashServer::m_Bw), MakeUintegerChecker<uint32_t>(1))
...
// Application Methods
  ...
  {
    // all connected clients' bitrates, have not considered situation of client leaving
    m_allClient[socket] = resolution;
    uint32_t totalRate = 0;
    uint32_t s_count = 0;
    double fenzi = 0.0;
    double fenmu = 0.0;
    for (std::map<Ptr<Socket>, uint32_t>::iterator it = m_allClient.begin(); it != m_allClient.end(); it++)
    {
        if (it->second != 0)
        {
            totalRate += it->second;
            s_count++;
            fenzi += it->second/1000000.0;
            fenmu += pow((it->second/1000000.0),2);
        }
    }

    double ineffi = 0.0;
    if (totalRate <= m_Bw)
        ineffi = double(m_Bw-totalRate)/m_Bw;
    else
        ineffi = 0;
    m_effiCoeff[Simulator::Now()] = ineffi;

    if (s_count > 0)
        m_fairCoeff[Simulator::Now()] = sqrt(1-pow(fenzi,2)/(s_count*fenmu));
        double fair_tmp = 0;
        int fair_count = 0;
        for (std::map<Time, double>::iterator it = m_fairCoeff.begin(); it != m_fairCoeff.end(); it++)
        {
            fair_tmp += it->second;
            fair_count++;
        }
    m_fairness = fair_tmp / fair_count;

    double effi_tmp = 0;
    double effi_count = 0;
    for (std::map<Time, double>::iterator it = m_effiCoeff.begin(); it != m_effiCoeff.end(); it++)
        if (it->first > Seconds(50))
        {
            effi_tmp += it->second;
            effi_count++;
        }
    if (effi_count > 0)
        m_efficiency = effi_tmp / effi_count;
    else
        m_efficiency = 1;

    // only this if needs to be commented out when not Estc.
/*    if (resolution > m_Bw/s_count)
    {
        uint32_t rates[] =
        ...
        uint32_t estcAve = m_Bw/s_count;
        if (estcAve >= rates[rates_size-1])
            resolution = rates[rates_size-1];
        else if (estcAve <= rates[0])
            resolution = rates[0];
        else
        {
            for (unsigned int i = 1; i < rates_size; i++)
                if (estcAve < rates[i])
                {
                    resolution = rates[i-1];
                    break;
                }
        }
        std::cout << Simulator::Now().GetSeconds() << " /Server /Bw: " << m_Bw << " s_count: " << s_count << " estcAve: " << estcAve
            << " birate: " << resolution << std::endl;
    }
*/
    ...
    for (uint32_t f_id = 0; f_id < MPEG_FRAMES_PER_SEGMENT; f_id++)
      {
        ...
        mpeg_header.SetTotalRate(totalRate);
        mpeg_header.SetClientNo(s_count);
    ...
    //std::cout << Simulator::Now().GetSeconds() << " /Server /" << " birate: " << resolution << std::endl;
    DataSend(socket, 0);
  }
  ...
  DashServer::GetStats()
  {
    std::cout <<  "infairness: " << m_fairness << "     inefficiency: " << m_efficiency << std::endl;
  }
