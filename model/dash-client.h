...
namespace ns3
{
  ...
  protected:
    ...
    void
    AddBitRate(Time time, double bitrate, uint32_t segId);
    ...
    uint32_t realBitRate;
    Time m_currDt;

    double m_targetDataRate;
    double m_smoothDataRate;
    std::map<uint32_t, uint32_t> m_bitSwitch;
  ...
  private:
    ...
    TypeId m_tid;
    TracedValue<Time> tr_currDt;
    TracedValue<uint32_t> tr_bitRate;
    ...
    std::map<Time, struct fetchRate> m_staRate;
    std::map<Time, double> m_staCoeff;
    double m_stability;
