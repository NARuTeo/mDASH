...
namespace ns3
{
  ...
  void
  MPEGHeader::SetTotalRate(uint32_t totalRate)
  {
    NS_LOG_FUNCTION(this);
    m_totalRate = totalRate;
  }
  uint32_t
  MPEGHeader::GetTotalRate(void) const
  {
    NS_LOG_FUNCTION(this);
    return m_totalRate;
  }
  ...
  void
  MPEGHeader::SetClientNo(uint32_t clientNo)
  {
    NS_LOG_FUNCTION(this);
    m_clientNo = clientNo;
  }
  uint32_t
  MPEGHeader::GetClientNo(void) const
  {
    NS_LOG_FUNCTION(this);
    return m_clientNo;
  }
  ...
  MPEGHeader::Serialize(Buffer::Iterator start) const
  {
    ...
    i.WriteHtonU32(m_totalRate);
    i.WriteHtonU32(m_clientNo);
  }
  uint32_t
  MPEGHeader::Deserialize(Buffer::Iterator start)
  {
    ...
    m_totalRate = i.ReadNtohU32();
    m_clientNo = i.ReadNtohU32();
